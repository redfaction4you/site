...
/**
 * The label, in one place so it cannot be phrased two ways.
 *
 * Short on purpose. It used to read "Illustration, generated by
 * gemini-3-pro-image. Not a photograph of the match.", which under a thumbnail
 * ran to three lines and was taller than the picture: a caption that size reads
 * as the image apologising for itself.
 *
 * Two words still does the job. Nobody needs telling twice that a picture of a
 * 2001 game character was not taken with a camera, and the model that made it is
 * on the row in `image_model` for anyone who wants it. What must not happen is no
 * label at all, which is the one version of this that would be dishonest.
 */
export const IMAGE_CAPTION = "AI interpretation";
